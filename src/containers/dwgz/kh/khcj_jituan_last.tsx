...
class KHCJ extends React.Component<P, S> {
  ...
  load() {
    client.testTasks.search(false, this.state.org ? this.state.org.id : 0, 0, this.state.startDate, this.state.endDate, (err: models.Error, res: Array<models.TestTask>) => {
    ...
    const listEl = this.state.tasks.sort((task: models.TestTask) => {
      ...
      return (
        <tr key={task.id}>
          ...
          <td className="center">
            <a onClick={this.props.onTableChange.bind(this, 'khcj_jituan_last_dwxq', task.id) } className="m2fm_abtn" href="javascript:;">党委详情</a>
          </td>
        </tr>
      )
    })
