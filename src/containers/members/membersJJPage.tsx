...
class MembersJJPage extends React.Component<P, S> {
  constructor(props) {
    super(props)
    this.state = {
      users: null,
  ...
  getListEl(isOp: boolean) {
    return this.state.users.map((user: models.User) => {
      ...
      if (isOp) {
        opEl = (
          <div>
            <a onClick={this.onEdit.bind(this, constants.WinTypes.MEMBERS_FPPYR, user.id)} href="javascript:;" className="m2fm_abtn">分配培养人</a>
            <a onClick={this.onEdit.bind(this, constants.WinTypes.MEMBERS_ZHUANCHU, user.id)} href="javascript:;" className="m2fm_abtn">转出</a>
            <a onClick={this.onEdit.bind(this, constants.WinTypes.MEMBERS_LWFZDX, user.id)} href="javascript:;" className="m2fm_abtn">列为发展对象</a>
            <Link className="m2fm_abtn" to={'/users/' + user.userName + '/jbxx?returnUrl=' + encodeURIComponent(constants.Links.MEMBERS_JJ)}>编辑</Link>
          </div>
        )
      }
      return (
        <tr key={user.id}>
          ...
          <td>{user.sex}</td>
          <td>{user.orgName}</td>
          <td><a onClick={this.onEdit.bind(this, constants.WinTypes.MEMBERS_FPPYR, user.id)} href="javascript:;" className="cor_red">查看</a></td>
          <td>{utils.Translate.toShortDate(user.toActivistDate)}</td>
          <td><a onClick={this.onEdit.bind(this, constants.WinTypes.MEMBERS_DKXXJL, user.id)} href="javascript:;" className="cor_red">党课学习记录</a></td>
          <td>{user.mobile}</td>
          <td>
            {opEl}
          </td>
  ...
  render() {
    ...
    let addBtnEl = null
    if (isOp) {
      addBtnEl = <Link to={constants.Links.MEMBERS_ADD + "?userType=" + encodeURIComponent('积极分子')} className="m2addBtn"><img src="/assets/images/m2btn.jpg" width="76" height="32" /></Link>
    }
    ...
    if (this.state.winType) {
      ...
      if (this.state.winType === constants.WinTypes.MEMBERS_DKXXJL) {
        ...
        formEl = <FPPYR isOp={isOp} user={user} org={this.props.orgState.org} onClose={this.onClose.bind(this)} />
      }
    }
    ...
    return (
      <div>
        ...
        <div className="m2fm_stop">
          ...
          <span className="m2fm_ss1">列为积极分子日期：</span>
