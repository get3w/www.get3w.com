...
export default class ZSCL extends React.Component<P, S> {
  ...
  load(userName: string) {
    client.records.search(userName, '政审材料', (err: models.Error, res: Array<models.Record>) => {
      if (res && res.length) {
  ...
  render() {
    ...
    if (this.state.mainType == 'list') {
      ...
      mainEl = <Add org={this.props.org} openMain={this.openMain.bind(this)} user={this.props.user} record={this.state.record} />
    }
    ...
    return (
      ...
      <div className="layerCon1 layerCon3 layerCon3b" style={{ height: 355, width: 518, marginLeft: '-259px', marginTop: '-177px' }}>
        ...
        <div className="layer_t">政审材料管理</div>
          {mainEl}
        </div>
      </div>
    )
